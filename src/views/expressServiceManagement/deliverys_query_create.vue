<template>
	<div class="all-height flex-column main-card">
		<el-tabs class="flex-column flex-1 main-card-tabs" v-model="tabPaneName" @tab-change="tabChange">
			<el-tab-pane
				title1="快递服务申请"
				:label="$t('menubaseExpressServiceApplication')"
			...
			<el-tab-pane
				title1="快递服务查询"
				:label="$t('menubaseExpressServiceQuery')"
				class="main-tab-pane-content all-height flex-column"
				name="1"
			>
...
const tableList1 = reactive({
	id: "/expressServiceManagement/deliverys_query_create.vue_zTable1",
	//请求属性设置
	httpAttribute: {
...
const tableList2 = reactive({
	id: "/expressServiceManagement/deliverys_query_create.vue_zTable2",
	//请求属性设置
	httpAttribute: {
		url: "/crm/deliverys/deliverys!selectDeliverysInfosByCond.action",
		root: "deliverysInfos",
...
const newDeliverysworkflowdetail = () => {
	deliverysworkflowdetailList.dmid = "";
	deliverysworkflowdetailList.workflowflag = "1";
...
const copy_handler = row => {
	...
	ElMessageBox({
		...
		let params = {
			jsonString: JSON.stringify({ deliverysInfos: row }),
			"cond.copynum": copynum.value
		};
		const res = await http.post("/crm/deliverys/deliverys!copyDeliverysInfos.action", qs.stringify(params));
		if (res) {
...
const batchDelete = ids => {
	ElMessageBox.confirm(i18n.t("Message_Confirmdelete"), i18n.t("reminder"), {
		...
		ids.forEach(item => {
			jsonString.deliverysInfos.push({
				dmid: item
			});
		});
		let params = {
		...
		const res = await http.post("/crm/deliverys/deliverys!deleteDeliverysInfos.action", qs.stringify(params));
		if (res) {
...
const Submit = row => {
	ElMessageBox.confirm(i18n.t("Message_ConfirmOrNotSubmit"), i18n.t("reminder"), {
		...
		const res = await http.post("/crm/deliverys/deliverys!submitDeliverysInfos.action", qs.stringify(params));
		if (res) {
