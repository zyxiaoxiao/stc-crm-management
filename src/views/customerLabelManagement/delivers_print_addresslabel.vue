	...
	<div class="all-height flex-column">
		<el-auto-resizer>
			<template #default="{ height, width }">
				<div class="flex-row" :style="{ height: height + `px`, width: width + `px` }">
					<div class="flex-column" :style="{ height: height + `px`, width: width - 400 + `px` }">
						<div class="el-card" style="min-height: 236px" :style="{ height: height / 2 + `px`, width: width - 410 + `px` }">
							<zTable ref="zTable1" :tableList="tableList1" @current-change="zTable1CurrentChange"></zTable>
						</div>
						<div class="flex-row" style="margin-top: 10px" :style="{ height: height / 2 + `px`, width: width - 408 + `px` }">
							<div
								class="el-card"
								style="margin-right: 10px"
								:style="{ height: height / 2 - 2 + `px`, width: (width - 408) / 2 + `px` }"
							>
								<zTable ref="zTable2" :tableList="tableList2" @current-change="zTable2CurrentChange"></zTable>
							</div>
							<div class="el-card" :style="{ height: height / 2 - 2 + `px`, width: (width - 408) / 2 + `px` }">
								<zTable style="" ref="zTable3" :tableList="tableList3" @current-change="zTable3CurrentChange"></zTable>
							</div>
						</div>
					</div>
					<div class="flex-column" style="min-height: 395px" :style="{ height: height + `px`, width: `400px` }">
...
const tableList1 = reactive({
	id: "/customerLabelManagement/delivers_print_addresslabel.vue_zTable1",
	isRadio: true, //设置为单选
	//请求属性设置
	httpAttribute: {
		url: "/mylims/enterpriseinfo/enterpriseinfo!selectCustomerBycorpright.action",
		root: "enterpriseInfos",
		baseParams: { "cond.auditflag": "2", "cond.discardflag": "1" }
	},
	//表格表头
	tableColumns: [
		{
...
const zTable3CurrentChange = (currentRow, oldCurrentRow) => {
	if (currentRow) {
		if (zTable1.value.selectList && zTable2.value.selectList && zTable3.value.selectList) {
			addressLabelList.corpdesc = zTable1.value.selectList[0].corpdesc;
			addressLabelList.contactdesc = zTable2.value.selectList[0].contactdesc;
			addressLabelList.tel = zTable2.value.selectList[0].tel;
			addressLabelList.address = currentRow.address;
		}
	}
};
