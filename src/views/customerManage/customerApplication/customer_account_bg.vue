...
const formData = reactive({
	accountbgid: props.condobj.accountbgid,
...
//根据 accountbgid 初始化数据源
const getFormData = async () => {
	await getIsmain();
	if (props.condobj.accountbgid) {
		let params = {
			"cond.corpid": props.condobj.corpid,
			"cond.accountbgid": props.condobj.accountbgid,
			"cond.corpbgid": props.condobj.corpbgid
		};
		const res = await http.post("/crm/account/crmaccount!selectCrmaccountbgInfoByCond.action", qs.stringify(params));
		for (let key in res.crmaccountbgInfos[0]) {
			//判定 crmaccountbgInfos 的key 是否存在 formData 的key
			if (Object.keys(formData).includes(key)) {
				formData[key] = res.crmaccountbgInfos[0][key];
			}
		}

		//判断是不是主账户
		if (res.crmaccountbgInfos[0]["ismain"] == 1) {
			ismain2 = true;
		}
	}
};
