					...
					<el-row :gutter="15" class="main-align-items-center">
						...
						<el-col :span="6">
							<el-form-item title1="英文名称" :label="$t('hotlinkpanelywmctitle')" prop="corpenglishname">
								<el-input type="text" clearable v-model.trim="formData.corpenglishname" :formatter="value => value.toUpperCase()">
								</el-input>
							</el-form-item>
						</el-col>
						<el-col :span="6">
					...
					<el-row :gutter="15" class="main-align-items-center">
						<el-col :span="6">
							<el-form-item title1="是否协议客户" :label="$t('itemtitlebase_userisdiscount')">
								<el-select v-model="formData.isdiscount" class="full-width-input" clearable filterable disabled>
						...
						<el-col :span="6">
							<el-form-item title1="折扣" :label="$t('columnappointmentdiscount')">
								<el-input type="text" clearable readonly v-model.number="formData.discount"></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="6">
					...
					<el-row :gutter="15" class="main-align-items-center">
						...
						<el-col :span="6">
							<el-form-item title1="归属级别" :label="$t('columnlevelattribution')">
								<el-select v-model="formData.deptlevel" class="full-width-input" clearable filterable disabled>
...
import customerAccountNew from "./customer_account_new.vue";
...
const getFormData = async () => {
	...
	for (let key in res.enterpriseInfo[0]) {
		...
		if (Object.keys(formData).includes(key)) {
			...
			if (key == "agentcode" && res.enterpriseInfo[0]["agentcode"]) {
				agentbrokerageDisabled.value = false;
			}
		}
	}
...
const save_handler = formEl => {
	...
	formEl.validate((valid, fields) => {
		if (valid) {
			//保存企业基本信息，前判断是否查询了客户信息, 并且 corpid 是空的
			if (existence && !formData.corpid) {
...
const tableList = reactive({
	id: "/customerManage/customerApplication/company_detail_new.vue_zTable1",
	//面初始化渲染完成 是否调请求
	isRequest: true,
	//表格工具栏
	tableToolbar: {
...
const tableList2 = reactive({
	id: "/customerManage/customerApplication/company_detail_new.vue_zTable2",
	//面初始化渲染完成 是否调请求
	isRequest: true,
	//表格工具栏
	tableToolbar: {
...
const tableList22 = reactive({
	id: "/customerManage/customerApplication/company_detail_new.vue_zTable22",
	//表格工具栏
	tableToolbar: {
		right: false
	},
...
const tableList3 = reactive({
	id: "/customerManage/customerApplication/company_detail_new.vue_zTable3",
	//面初始化渲染完成 是否调请求
	isRequest: true,
	//表格工具栏
	tableToolbar: {
...
const tableList33 = reactive({
	id: "/customerManage/customerApplication/company_detail_new.vue_zTable33",
	//表格工具栏
	tableToolbar: {
		right: false
	},
...
const tableList4 = reactive({
	id: "/customerManage/customerApplication/company_detail_new.vue_zTable4",
	//面初始化渲染完成 是否调请求
	isRequest: true,
	//表格工具栏
	tableToolbar: {
...
const tableList5 = reactive({
	id: "/customerManage/customerApplication/company_detail_new.vue_zTable5",
	//开启编辑
	edit: true,
	//面初始化渲染完成 是否调请求
	isRequest: true,
