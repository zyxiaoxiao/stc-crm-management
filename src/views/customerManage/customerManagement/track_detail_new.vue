<template>
	<div class="all-height flex-column">
		...
		<div v-dialogStretching>
			...
			>
				<selectCorpForTrack :condobj="selectCorpForTrackList" />
			</ZDialog>
		</div>
...
import selectCorpForTrack from "../customerSelete/selectCorpForTrack.vue";
...
const save_handler = formEl => {
	...
	formEl.validate(async (valid, fields) => {
		if (valid) {
			...
			const res = await http.post("/mylims/trackinfo/track!saveTrackInfo.action", qs.stringify(params));
			if (res) {
...
const tableList = reactive({
	id: "/customerManage/customerManagement/track_detail_new.vue_zTable1",
	//面初始化渲染完成 是否调请求
	isRequest: true,
	//表格工具栏
	tableToolbar: {
