<template>
	<div class="all-height flex-column main-card" style="padding: 10px">
		...
		<div v-dialogStretching>
			...
			>
				<potentialtrackdetailnew :condobj="addTrackingRecordList" />
			</ZDialog>
		</div>
	</div>
</template>
...
import potentialtrackdetailnew from "./potential_track_detail_new.vue";
...
const tableList8 = reactive({
	id: "/customerManage/potentialCustomer/potential_track_list.vue_zTable8",
	//请求属性设置
	httpAttribute: {
		url: "/mylims/trackinfo/track!selectPotentialTrackInfoAllByCond.action",
		root: "trackInfos",
		baseParams: {}
	},
	//表格表头
...
const abandoned_handler = selectList => {
	...
	for (let row of selectList) {
		if (row.state == "2") {
			ElMessage.warning(row.companyname + i18n.t("message_error_customers3"));
			return;
		} else if (row.state == "1" || row.state == "3") {
			ElMessage.warning(row.companyname + i18n.t("message_error_customers4"));
			return;
		} else {
			let obj = {};
			for (let key in row) {
	...
	ElMessageBox.confirm(i18n.t("Message_AbandonedYesNo"), i18n.t("reminder"), {
		...
		const res = await http.post(
			"/mylims/trackinfo/track!updateTrackInfo.action",
			qs.stringify({ jsonString: JSON.stringify({ trackInfos: listData }) })
		);
		if (res) {
...
const enable_handler = selectList => {
	...
	for (let row of selectList) {
		if (row.state == "1" || row.state == "3") {
			ElMessage.warning(row.reocrdercode + i18n.t("messageEnabledError"));
		} else if (row.state == "0") {
			ElMessage.warning(row.reocrdercode + i18n.t("message_error_customers5"));
			return;
		} else {
			let obj = {};
			for (let key in row) {
