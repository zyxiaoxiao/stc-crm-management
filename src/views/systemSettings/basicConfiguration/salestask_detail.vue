	...
	<div class="all-height flex-column">
		...
		>
			<el-row :gutter="15" class="main-align-items-center">
				<el-col :span="8">
					<el-form-item title1="销售任务编号" :label="$t('columnbasesalestaskcode')">
						<el-input v-model="formData.salestasknum" disabled> </el-input>
					</el-form-item>
				</el-col>
				<el-col :span="8">
				...
				<el-col :span="8">
					<el-form-item title1="任务总额(HKD)" :label="$t('columnbasesalestotalsales')">
						<el-input v-model="formData.totalsales" disabled> </el-input>
					</el-form-item>
				</el-col>
				<el-col :span="8">
					<el-form-item title1="销售名称" :label="$t('columnbasesalesdescus')" prop="salesmandesc">
						<el-input v-model.trim="formData.salesmandesc" readonly :disabled="formDisabledList.salesmandesc">
							<template #append>
								<el-button-group class="flex-row">
									<el-button :disabled="formDisabledList.salesmandesc" @click="selectSalesmandesc" icon="ZoomIn" />
									<el-button :disabled="formDisabledList.salesmandesc" @click="delSalesmandesc" type="primary" icon="ZoomOut" />
								</el-button-group>
							</template>
						</el-input>
					</el-form-item>
				...
				<el-col :span="8">
					<el-form-item title1="销售编码" :label="$t('columnbasesalescode')">
						<el-input v-model.trim="formData.salesmancode" disabled> </el-input>
					</el-form-item>
				</el-col>
				<el-col :span="8">
					<el-form-item title1="销售名称(中)" :label="$t('columnbasesalesdesczh')">
						<el-input v-model.trim="formData.salesmandesc_cn" disabled> </el-input>
					</el-form-item>
				</el-col>
				<el-col :span="24">
					<el-form-item title1="描述" :label="$t('itemtitlebase_codecodedesc')">
						<el-input type="textarea" v-model.trim="formData.reamrk"> </el-input>
					</el-form-item>
				</el-col>
			</el-row>
			<el-row :gutter="15" class="main-align-items-center">
		...
		<zTable style="overflow: auto" ref="zTable1" :tableList="tableList1">
			<template #tableHeaderLleft="scope">
				<el-button size="small" type="primary" icon="Edit" plain :disabled="!formData.salestaskid" @click="AddComprehensive">{{
					$t("menu_new")
				}}</el-button>
				<el-button size="small" type="primary" icon="Edit" plain :disabled="!formData.salestaskid" @click="addInBulk">{{
					$t("menu_addInBulk")
				}}</el-button>
				<el-button
					size="small"
...
const formData = reactive({
	salestaskid: props?.condobj?.salestaskid
});

const rules = reactive({
	year: [{ required: true, message: i18n.t("rulesPropMessage") }],
	salesmandesc: [{ required: true, message: i18n.t("rulesPropMessage") }]
});
...
const selectheadmanClose = () => {
	...
	if (arrayList && arrayList.length > 0) {
		...
		formData.salesmandesc_cn = arrayList[0].userdesc_cn;
	}
};
//清除选择的销售
const delSalesmandesc = () => {
	formData.salesmandesc = "";
	formData.salesmancode = "";
	formData.salesmandesc_cn = "";
};

//数据源处理
const dataSourceProcessing = data => {
	for (let key in data.salestaskInfo[0]) {
		if (props.condobj.workflowflag == "1") {
		...
		formData[key] = data.salestaskInfo[0][key];
	}
};
//根据 salestaskid 初始化数据源
const getFormData = async () => {
	const res = await http.post(
		"/crm/salestask/salestask!selectSalestaskInfoById.action",
		qs.stringify({
			jsonString: JSON.stringify({
				salestaskInfo: {
					salestaskid: formData.salestaskid
				}
			})
		})
	);
...
const save_handler = formEl => {
	...
	formEl.validate(async (valid, fields) => {
		if (valid) {
			let srtUrl = "/crm/salestask/salestask!insertSalestaskInfo.action";
			if (formData.salestaskid) {
				srtUrl = "/crm/salestask/salestask!updateSalestaskInfo.action";
			}
			let jsonString = {
				salestaskInfo: formData
			};
			let params = {
...
const tableList1 = reactive({
	id: "/systemSettings/basicConfiguration/salestask_detail.vue_zTable1",
	//当前表格是否可编辑
	...
	//请求属性设置
	httpAttribute: {
		url: "/crm/salestask/salestaskdetail!selectSalestaskdetailInfoByCond.action",
		root: "salestaskdetailInfos",
		baseParams: {}
	},
	//表格表头
	tableColumns: [
		...
		{
			title: "销售任务额",
			label: "columnbasesalestaskwell_hkd",
			prop: "salesmoney",
			type: "Number",
			width: "160",
			min: 0,
...
const AddComprehensive = () => {
	tableList1.httpAttribute.baseParams["cond.salestaskid"] = formData.salestaskid;
	zTable1.value.addRowData({ salestaskid: formData.salestaskid, salesmoney: 0 });
};
...
const addInBulk_save = async () => {
	...
	let jsonString1 = {
		cond: {
			salestaskid: formData.salestaskid,
			date_start: addInBulkList.startMonth + "-01",
			date_end: addInBulkList.endMonth + "-01"
		}
	};
	let params1 = {
		jsonString: JSON.stringify(jsonString1)
	};
	const res1 = await http.post("/crm/salestask/salestaskdetail!saveSalestaskdetailInfoWithBatch.action", qs.stringify(params1));
	if (res1) {
		tableList1.httpAttribute.baseParams["cond.salestaskid"] = formData.salestaskid;
...
const delComprehensive = ids => {
	ElMessageBox.confirm(i18n.t("Message_Confirmdelete"), i18n.t("reminder"), {
		...
		let jsonString = {
			salestaskdetailInfos: []
		};
		ids.forEach(item => {
			jsonString.salestaskdetailInfos.push({
				id: item
			});
		});
		let params = {
		...
		const res = await http.post("/crm/salestask/salestaskdetail!deleteSalestaskdetailInfo.action", qs.stringify(params));
		if (res) {
...
const save_gradeInfos = async () => {
	...
	if (EditSelectList.length > 0) {
		for (let obj of EditSelectList) {
			let ttime = obj.ttime;
			let salesmoney = obj.salesmoney;
			if (!ttime) {
				//月份不能为空
				ElMessage.warning(i18n.t("message_null_month"));
				return false;
			}
			if (!salesmoney && salesmoney < 0) {
				//任务额不能为空
		...
		let jsonString1 = {
			salestaskdetailInfos: EditSelectList
		};
		let params1 = {
			jsonString: JSON.stringify(jsonString1)
		};
		const res1 = await http.post(
			"/crm/salestask/salestaskdetail!saveSalestaskdetailInfo.action?salestaskid=" + formData.salestaskid,
...
watch(
	...
	(newValue, oldValue) => {
		...
		for (let row of newValue) {
			salesmoney = salesmoney + row.salesmoney;
		}
		formData.totalsales = salesmoney;
	},
	{ deep: true } //深度监听
);
